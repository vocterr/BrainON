// FILE: app/rezerwacja-anulowana/page.tsx

"use client";

import { motion } from 'framer-motion';
import { FiXCircle, FiCalendar, FiHome } from 'react-icons/fi';
import { useRouter } from 'next/navigation';
import Link from 'next/link';
import { useIsMobile } from '@/lib/useIsMobile';

export default function RezerwacjaAnulowanaPage() {
    ...
    return (
        <main className="w-full min-h-screen flex items-center justify-center bg-slate-900 text-white font-chewy p-4 relative overflow-hidden">
            <div className="absolute inset-0 z-0 opacity-40 hidden md:block">
                <motion.div animate={{ rotate: 360 }} transition={{ duration: 70, repeat: Infinity, ease: "linear" }} className="absolute top-[-40%] left-[-40%] w-[100vw] h-[100vw] bg-gradient-to-br from-red-800/50 via-orange-700/40 to-transparent rounded-full blur-3xl" />
                <motion.div animate={{ rotate: -360 }} transition={{ duration: 80, repeat: Infinity, ease: "linear" }} className="absolute bottom-[-40%] right-[-40%] w-[100vw] h-[100vw] bg-gradient-to-tl from-yellow-800/40 to-transparent rounded-full blur-3xl" />
            </div>
            ...
            <motion.div
                ...
                initial="hidden"
                animate="visible"
                className="w-full max-w-2xl p-8 sm:p-12 text-center rounded-3xl bg-slate-800/50 border border-red-500/30 backdrop-blur-lg shadow-2xl shadow-red-500/10 relative z-10"
            >
                ...
                <motion.h1 variants={itemVariants} className="text-4xl sm:text-5xl md:text-6xl bg-clip-text text-transparent bg-gradient-to-r from-red-400 to-orange-400 mb-4">
                    Rezerwacja Anulowana
                </motion.h1>

                <motion.p variants={itemVariants} className="font-sans text-lg text-slate-300 max-w-md mx-auto mb-10">
                    Proces płatności został przerwany lub anulowany. Twój termin nie został zarezerwowany, a karta nie została obciążona.
                </motion.p>
