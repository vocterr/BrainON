...
// KROK 1: Poprawny, standardowy import funkcji `signIn`
...
export default function LoginPage() {
    ...
    useEffect(() => {
        ...
        if (callbackError) {
            // Możemy tu dodać bardziej szczegółowe błędy w przyszłości
            setError('Logowanie nie powiodło się. Spróbuj ponownie.');
        }
    }, [searchParams]);

    // KROK 2: Uproszczona i w pełni funkcjonalna logika z użyciem signIn
    const handleLogin = async (e: FormEvent) => {
        ...
        try {
            // Używamy funkcji signIn z Next-Auth dla logowania "credentials"
            const result = await signIn('credentials', {
                redirect: false, // Ważne: nie przekierowujemy automatycznie, obsługujemy to ręcznie
                email: email,
                password: password,
            });

            if (result?.error) {
                // Jeśli Next-Auth zwróci błąd (np. z naszego API), wyświetlamy go
                // Wartość 'result.error' jest tutaj zazwyczaj generyczna, więc ustawiamy własny komunikat
                setError("Nieprawidłowy email lub hasło.");
            } else if (result?.ok) {
                // Jeśli logowanie się udało, przekierowujemy na stronę główną
                router.push('/');
            }
        } catch (err) {
    ...
    const handleGoogleLogin = () => {
        setIsLoading(true);
        // Wywołujemy signIn z providerem 'google'. Next-Auth zajmie się resztą.
    ...
    return (
        <main className="w-full min-h-screen flex items-center justify-center bg-slate-900 text-white font-chewy p-4 relative overflow-hidden">
            ...
            <motion.div variants={containerVariants} initial="hidden" animate="visible" className="w-full max-w-md p-8 sm:p-12 rounded-3xl bg-slate-800/50 border border-purple-500/30 backdrop-blur-lg shadow-2xl relative z-10">
